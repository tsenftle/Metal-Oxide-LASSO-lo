function [D_f2,head_f2,unit_out,n_c2] = optc2p(D_p,header,unit_in,opt_flag)
            ...
            parfor i=1:sz2(1)
                switch opt_flag
                    ...
                    case '*abs'       
                        ...
                        unit_out(i)=unit_in(n_c2(i,1))+unit_in(n_c2(i,2));
                        head_f2(i)=strcat('|',header(n_c2(i,1)),'*',header(n_c2(i,2)),'|');
                    case '*absI' % Special multiplication to avoid columns withe value of 1 (happens with inverse maultiplications)
                        D_temp=D_p(:,n_c2(i,1)).*D_p(:,n_c2(i,2));
                        if(all(abs(D_temp-1.0))>1e-8)
                            D_f2(:,i)=abs(D_p(:,n_c2(i,1)).*D_p(:,n_c2(i,2)));
                            unit_out(i)=unit_in(n_c2(i,1))+unit_in(n_c2(i,2));
                            head_f2(i)=strcat('|',header(n_c2(i,1)),'*',header(n_c2(i,2)),'|');
                        end
